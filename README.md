...
**Status:** Google OAuth (#1), the Mapping doctype (#2), Frappe → Sheets push sync (#3), Sheets → Frappe pull sync (#4), and conflict detection (#5) are all done. See [sheets.txt](../sheets.txt) for the full feature spec this connector is being built against.
...
- **Conflict detection** (done, #5) — `Google Sheets Sync State` tracks the last-agreed value per (mapping, record, field). If both the Frappe value and the Sheet value have moved away from that baseline to different values, neither side is applied; the field is flagged and both real values recorded, staying flagged until an admin clears it from the Mapping form's "View Conflicts" button. A field where only one side changed still applies normally.
...
| `alaiy_os_connector_google_sheets/doctype/google_sheets_sync_log/` | One row per sync run, with a `mapping` link field and a `conflict_count`; `.js` list view color-codes status. |
| `alaiy_os_connector_google_sheets/doctype/google_sheets_sync_state/` | One row per (mapping, record, field): the last value both sides agreed on, and whether it's currently flagged as an unresolved conflict. |
...
- [x] Pull sync runs manually and produces a `success` log with sane counters.
- [x] Test Connection succeeds against a real Google account and the connector card flips to "Connected".
- [ ] A real conflict (edit the same field on both sides between syncs) gets flagged, not silently overwritten -- not yet tested live.
