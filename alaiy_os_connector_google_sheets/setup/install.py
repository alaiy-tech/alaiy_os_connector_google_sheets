...
def sync_connector_registry():
    ...
    from alaiy_os_connector_google_sheets.connector_meta import connector_meta
...
def _update_alaiy_os_sidebar():
    ...
    try:
        ...
        frappe.log_error(
            title="Google Sheets connector: sidebar update failed",
...
def _fix_settings_as_single():
    ...
    frappe.db.sql(
        ...
        "WHERE name='Google Sheets Connector Settings' AND issingle=0"
    )
    frappe.db.commit()
...
def _ensure_custom_fields(doctype, fields):
    for f in fields:
        ...
        cf.module = "Alaiy Os Connector Google Sheets"
